...
> **加一本新教材（LEVEL 2~4）**：把词表放进 `docs/vocabulary_files/LEVEL N/`，跑 `python codes/import_textbook_excel.py N` 一条命令导入。`level-2/` `level-3/` `level-4/` 目录**已预建好**，前端 LEVEL 按钮、进度分组、单元列表全自动生成，**不用改代码**。格式与坑见 [DATA_TEMPLATE.md](codes/web/src/content/japanese/DATA_TEMPLATE.md) 1.5 与 AGENTS.md 3.4。
...
|------|----|----|------|
| `choice` 语法选择 | 5 | 2 | ✅ 有数据 |
| `translation` 翻译 | 3 | 0 | ✅ 有数据 |
| `fill` 语法词填空 | 0 | 0 | ⚠️ **无数据** |
| `reading` 读音练习 | 0 | 0 | ⚠️ **无数据** |

⚠️ **已知体验问题（2026-09-19 核实）**：界面上四个题型按钮**全部可选**（`Exercise.vue` 的 `typeOptions`），但后两类没有数据，选中后是静默什么都不发生（`if (exercises.length === 0) return`，连「暂无题目」的提示都没有），看上去像按钮坏了。补数据之前建议先加个空状态提示。

> 题库由人工整理，后续将持续补充。格式见 [[2026-09-15-todo-list]] 第五节。
...
- [ ] **练习题库数据补充**（当前仅 10 题，`fill` / `reading` 数据待人工整理，格式见 [[2026-09-15-todo-list]] 第五节）
- [x] 隐私政策运营者说明（2026-09-16：写「由个人开发者运营」+ ICP 备案号，不直陈真名）
- [x] 匿名用户进度刷新即丢的修复（2026-09-19，见 §6 v2.6.2）
- [x] `/user` 路由补 `requiresAuth`（2026-09-19）
- [x] 数据库备份脚本 + 每日 cron（2026-09-19）
- [ ] 语音播放（TTS）—— 用户决定暂缓，先补数据、打磨细节、上线看效果后再议
...
| v2.6.2 | 2026-09-19 | 匿名用户进度丢失修复、`/user` 补鉴权、数据库备份脚本 + 每日 cron |

### v2.6.2 变更（2026-09-19，已部署）

- **修复：匿名用户进度刷新即丢**。`App.vue` 原本只在有 token 时才初始化，而初始化是唯一从 `localStorage` 读回进度的路径；匿名用户内存里始终为空，下一次保存就把本地进度整个覆盖掉。**错题本与练习历史同样中招**。改为「先用 `localStorage` 垫底 → 登录后以服务端数据覆盖」，初始化函数随之改名 `initSRS` / `initExercise`
- ⚠️ 登录/注册成功后**不再**调用这两个初始化 —— 新账号服务端为空，调用会把匿名试用的进度覆盖掉
- 退出登录后重新读回本机数据，避免内存清空后被误覆盖
- **修复：`/user` 漏了 `requiresAuth`**，未登录可直接进入用户中心
- **新增：数据库备份**（此前线上零备份，且数据压在 WAL 里）。`server/scripts/backup-db.mjs` + cron 每天 03:17，产出留在 `/opt/japanese-learning/backups/` 14 份，含 `integrity_check` 校验与轮转

> 首页 `/` 保持公开（有意为之——匿名可看欢迎页与入口卡片），`/privacy`、`/contact` 因备案要求必须公开。
