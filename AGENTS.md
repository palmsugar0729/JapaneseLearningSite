...
### v2.6.1（✅ 2026-09-19 已部署上线，待浏览器验收）
...
- [x] 服务端一次性迁移清掉旧 `tb1-*` SRS 进度（线上清掉 6 行，因新旧 ID 同形，靠 `meta` 表标记位区分）
- [x] **前后端同时部署**（满足「后端必须与新前端一起上」的约束，否则旧前端会把 `tb1-*` 旧记录推回来）
- [ ] **浏览器验收**
...
- [x] 隐私政策运营者说明（2026-09-16）——个人备案主体为真名，页面上改用「由个人开发者运营」+ ICP 备案号作可追溯标识
